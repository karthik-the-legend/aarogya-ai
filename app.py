# ================================================================
# app.py — Aarogya AI — Streamlit Cloud Entry Point
# ================================================================

import os, sys, io, time, warnings, logging, threading
...
.hero {
    ...
    border: 1px solid #1e40af33; border-radius: 16px;
...
.badge-red    { background:#ff174418; color:#ff1744; border:1px solid #ff174440; padding:4px 14px; border-radius:100px; font-size:0.75rem; font-weight:600; display:inline-block; margin-top:8px; animation:pulse 1.5s infinite; }
...
with st.sidebar:
    ...
    selected_lang_display = st.selectbox("lang", options=list(LANG_MAP.keys()), label_visibility="collapsed")
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.warning("⚠️ General health info only — not a diagnosis.")
...
if uploaded is not None:
    with st.spinner("🎙️ Transcribing audio..."):
        try:
            ...
            v_lang  = transcript_data.get('lang_code', 'en')
            v_lname = transcript_data.get('language', 'English')

            st.markdown(f"""
            <div style="background:#0f172a;border:1px solid #1e40af33;border-radius:10px;padding:12px 16px;margin:12px 0">
                <div style="font-size:0.72rem;color:#64748b;margin-bottom:4px">🎙️ WHISPER · {v_lname}</div>
                <div style="color:#e2e8f0">{v_query}</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with st.chat_message("assistant"):
                with st.spinner("Searching..."):
                    vr = ask_pipeline(v_query, v_lang, v_lname)
                v_ans    = vr.get("answer","")
                v_triage = vr.get("triage_level","green")
                st.markdown(v_ans)
                emoji, cls, label = TRIAGE_CONFIG.get(v_triage, TRIAGE_CONFIG["green"])
                st.markdown(f'<span class="{cls}">{emoji} {label}</span>', unsafe_allow_html=True)
                st.caption(f"⏱️ {vr.get('latency_ms',0)}ms")
                v_audio = text_to_speech(v_ans, v_lang)
                if v_audio:
                    st.audio(v_audio, format="audio/mp3")
                    st.download_button("⬇️ Download audio", v_audio, "response.mp3", "audio/mp3")

            st.session_state.messages.extend([
                {"role":"user","content":f"🎙️ {v_query}"},
                {"role":"assistant","content":v_ans,"triage":v_triage,"latency":vr.get("latency_ms",0),"sources":vr.get("sources",[])}
            ])
        except Exception as e:
