...
/* ══════════════════════════════════════════════
   SIDEBAR — native toggle preserved, gap fixed
   ══════════════════════════════════════════════ */
...
/* ── Native collapse button — leave it FULLY alone so clicks work ── */
/* Only nudge its visual position slightly */
...
section[data-testid="stSidebar"][aria-expanded="false"] ~ .main .block-container {
    ...
    padding-left: 3rem !important;   /* clears the toggle button */
}
...
if uploaded is not None:
    with st.spinner("🎙️ Transcribing audio..."):
        try:
            ...
            r = requests.post(f"{API_BASE}/ask-voice", files=files, timeout=60)
            r.raise_for_status()
            data = r.json()
            ...
            st.session_state.messages.extend([
                {"role": "user",      "content": f"🎙️ {transcript.get('text','')}"},
