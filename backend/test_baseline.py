...
plain_safe = any(w in plain_lower for w in ['avoid', 'do not', 'should not', 'paracetamol'])
rag_safe   = any(w in rag_lower   for w in ['avoid', 'do not', 'should not', 'paracetamol'])

print(f"  Plain LLM warns against ibuprofen: {'✅ YES' if plain_safe else '❌ NO — DANGEROUS'}")
print(f"  RAG warns against ibuprofen:       {'✅ YES' if rag_safe   else '❌ NO — DANGEROUS'}")
