...
for kw in [kw1, kw2]:
    if kw not in content:
        content = content.replace(target, f'    "{kw}",\n{target}')
        print(f"Added: {kw}")
    else:
...
print(
    f'Test 1: {r1.level.value} (expected red) - {"PASS" if r1.level.value == "red" else "FAIL"}'
)
print(
    f'Test 2: {r2.level.value} (expected red) - {"PASS" if r2.level.value == "red" else "FAIL"}'
)
